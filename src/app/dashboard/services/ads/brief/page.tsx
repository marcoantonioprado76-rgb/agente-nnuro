...
const Loader = () => (
    <div className="flex items-center justify-center min-h-[60vh]">
        <div className="w-8 h-8 border-2 border-[#E4E9F0] border-t-[#147e95] rounded-full animate-spin" />
    </div>
)
        ...
        <div className="px-4 md:px-6 xl:px-10 pt-6 max-w-screen-xl 2xl:max-w-screen-2xl mx-auto pb-24 text-white" style={{ background: 'radial-gradient(120% 55% at 50% -5%, rgba(0,229,208,0.12), rgba(255,255,255,0) 55%), radial-gradient(90% 55% at 100% 108%, rgba(106,53,217,0.10), rgba(255,255,255,0) 60%), linear-gradient(180deg, #273842 0%, #1a262f 100%)', minHeight: 'calc(100vh - 2rem)', borderRadius: 24, border: '1px solid rgba(255,255,255,0.08)', boxShadow: '0 30px 60px -30px rgba(8,22,36,0.5)' }}>
            ...
            {!brief ? (
                <>
                    ...
                    <p className="text-[11px] font-bold text-[#147e95] mb-2.5 flex items-center gap-1.5">
                    ...
                    {inputMode === 'text' && (
                        <div className="space-y-4">
                            <textarea value={text} onChange={e => setText(e.target.value)} rows={9}
                                ...
                                className="w-full bg-[#212e38] border border-white/20 rounded-2xl px-4 py-3 text-sm text-white resize-none focus:outline-none focus:border-white/10 placeholder:text-white/25 leading-relaxed" />
                            ...
                            {!audioBlob ? (
                                <div className="flex flex-col items-center gap-6">
                                    <button onClick={recording ? stopRecording : startRecording}
                                        className={`relative w-28 h-28 rounded-full flex items-center justify-center transition-all ${recording ? 'bg-[#F87171] shadow-[0_0_60px_rgba(239,68,68,0.5)]' : 'bg-[#147e95] shadow-[0_0_60px_rgba(0,229,208,0.4)]'}`}>
                                ...
                                <div className="flex flex-col items-center gap-4">
                                    ...
                                    <div className="flex gap-3">
                                        ...
                                        <button onClick={transcribeAudio} disabled={transcribing} className="px-6 py-2 rounded-xl bg-[#147e95] text-white text-sm font-bold hover:bg-[#147e95] disabled:opacity-50 transition-all flex items-center gap-2">
                ...
                <div className="space-y-4">
                    ...
                    {(() => { const b = brief as any; const cat = AD_CATEGORIES.find(c => c.id === b.category); return (
                        <div className="p-4 rounded-2xl space-y-3" style={{ background: 'rgba(0,229,208,0.06)', border: '1px solid rgba(255,255,255,0.08)' }}>
                            <p className="text-[11px] font-bold text-[#147e95] flex items-center gap-1.5">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-[11px]">
                                {Array.isArray(b.benefits) && b.benefits.length > 0 && (
                                    <div><p className="text-white/30 font-bold uppercase mb-1">Beneficios</p><ul className="space-y-0.5">{b.benefits.map((x: string, i: number) => <li key={i} className="text-white/60 flex gap-1.5"><span className="text-[#147e95]">✓</span>{x}</li>)}</ul></div>
                                )}
                    ...
                    <div className="flex gap-3 pt-2">
                        ...
                        <button onClick={saveBrief} disabled={saving} className="flex-1 py-3 rounded-2xl bg-gradient-to-r from-[#1fb8bb] via-[#147e95] to-[#233B8F] text-white text-sm font-bold hover:opacity-90 disabled:opacity-50 transition-all flex items-center justify-center gap-2">
    ...
    return (
        <div className="px-4 md:px-6 xl:px-10 pt-6 max-w-screen-xl 2xl:max-w-screen-2xl mx-auto pb-24 text-white">
            ...
            <div className="flex gap-3 mt-6">
                ...
                <button onClick={save} disabled={saving} className="flex-1 py-3 rounded-2xl bg-gradient-to-r from-[#1fb8bb] via-[#147e95] to-[#233B8F] text-white text-sm font-bold hover:opacity-90 disabled:opacity-50 transition-all flex items-center justify-center gap-2">
    ...
    return (
        <div className="space-y-5">
            ...
            <Section title="Audiencia">
                ...
                <div>
                    <label className="text-xs font-bold text-white/40 uppercase tracking-widest block mb-2">Ubicaciones <span className="text-[#147e95]/60 font-normal lowercase">· detectadas por IA, podés ajustarlas</span></label>
            ...
            {((brief as any).category || (brief as any).benefits || (brief as any).adConcepts) && (
                <Section title="Enfoque del Anuncio (IA)">
                    <div>
                        <label className="text-xs font-bold text-white/40 uppercase tracking-widest block mb-2">Categoría <span className="text-[#147e95]/60 font-normal lowercase">· detectada por IA, podés cambiarla</span></label>
                        <select value={(brief as any).category || 'otro'} onChange={e => onChange({ ...brief, category: e.target.value } as any)} className="w-full bg-[#212e38] border border-white/20 rounded-xl px-3 py-2.5 text-sm text-white focus:outline-none focus:border-white/10 [&>option]:bg-[#212e38]">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div className="md:col-span-2 grid grid-cols-3 gap-3">
                            <div>
                                ...
                                <input type="number" min={18} max={65} value={(brief as any).targetAgeMin ?? ''} onChange={e => onChange({ ...brief, targetAgeMin: e.target.value ? Number(e.target.value) : undefined } as any)}
                                    placeholder="18" className="w-full bg-[#212e38] border border-white/20 rounded-xl px-3 py-2.5 text-sm text-white focus:outline-none focus:border-white/10" />
                            </div>
                            <div>
                                ...
                                <input type="number" min={18} max={65} value={(brief as any).targetAgeMax ?? ''} onChange={e => onChange({ ...brief, targetAgeMax: e.target.value ? Number(e.target.value) : undefined } as any)}
                                    placeholder="65" className="w-full bg-[#212e38] border border-white/20 rounded-xl px-3 py-2.5 text-sm text-white focus:outline-none focus:border-white/10" />
                            </div>
                            <div>
                                <label className="text-xs font-bold text-white/40 uppercase tracking-widest block mb-2">Género</label>
                                <select value={(brief as any).targetGender || 'all'} onChange={e => onChange({ ...brief, targetGender: e.target.value } as any)} className="w-full bg-[#212e38] border border-white/20 rounded-xl px-3 py-2.5 text-sm text-white focus:outline-none focus:border-white/10 [&>option]:bg-[#212e38]">
                        ...
                        <div>
                            ...
                            <select value={(brief as any).positioning || ''} onChange={e => onChange({ ...brief, positioning: e.target.value } as any)} className="w-full bg-[#212e38] border border-white/20 rounded-xl px-3 py-2.5 text-sm text-white focus:outline-none focus:border-white/10 [&>option]:bg-[#212e38]">
                                <option value="">—</option>
            ...
            <Section title="Estrategia">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <select {...field('primaryObjective')} className="w-full bg-[#212e38] border border-white/20 rounded-xl px-3 py-2.5 text-sm text-white focus:outline-none focus:border-white/10 [&>option]:bg-[#212e38]">
    ...
    return (
        <div>
            ...
            <div className="flex flex-wrap gap-2 mb-2">
                {items.map((item, i) => (
                    <span key={i} className="flex items-center gap-1.5 text-xs bg-[#147e95]/10 border border-white/10 text-[#147e95] px-3 py-1.5 rounded-full font-medium">
                        {item}
                        <button onClick={() => onChange(items.filter((_, j) => j !== i))} className="text-[#147e95] hover:text-[#F87171]">×</button>
                    </span>
                ))}
            </div>
            <div className="flex gap-2">
                <input value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && (e.preventDefault(), add())}
                    placeholder={placeholder} className="flex-1 bg-[#212e38] border border-white/20 rounded-xl px-3 py-2 text-sm text-white focus:outline-none focus:border-white/10 placeholder:text-white/30" />
                <button onClick={add} className="px-3 py-2 bg-white/5 border border-white/10 rounded-xl text-xs font-bold hover:bg-[#147e95]/20 transition-all">+ Agregar</button>
            </div>
        </div>
    )
}
...
function InputField({ label, value, onChange, placeholder }: { label: string; value: string; onChange: any; placeholder?: string }) {
    return (
        <div>
            <label className="text-xs font-bold text-white/40 uppercase tracking-widest block mb-2">{label}</label>
            <input value={value} onChange={onChange} placeholder={placeholder} className="w-full bg-[#212e38] border border-white/20 rounded-xl px-3 py-2.5 text-sm text-white focus:outline-none focus:border-white/10 placeholder:text-white/30" />
        </div>
    )
}
...
function TextareaField({ label, value, onChange }: { label: string; value: string; onChange: any }) {
    return (
        <div>
            <label className="text-xs font-bold text-white/40 uppercase tracking-widest block mb-2">{label}</label>
            <textarea value={value} onChange={onChange} rows={3} className="w-full bg-[#212e38] border border-white/20 rounded-xl px-3 py-2.5 text-sm text-white resize-none focus:outline-none focus:border-white/10 leading-relaxed" />
        </div>
    )
}
