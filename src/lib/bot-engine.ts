...
- mensaje4: sin límite (burbuja opcional, corta).` : ''
...
Reglas de mensajes (BURBUJAS de WhatsApp — hablás como una persona real):
- Cada campo mensaje1/mensaje2/mensaje3/mensaje4 es UNA burbuja: 1 sola idea, corta y directa (1–2 líneas).
- VOS decidís dónde cortar, como cortaría un humano por idea (NO por punto ni coma a la fuerza).
- mensaje1: SIEMPRE va (la idea principal, extremadamente corta).
- mensaje2/mensaje3/mensaje4: OPCIONALES. Úsalos solo si aportan (una pregunta, un dato clave). Si no, dejá "".
- En la mayoría de turnos alcanza con mensaje1 (y a veces mensaje2 con una pregunta corta).
- ANDÁ AL GRANO: no des contexto de más, no expliques lo que no te pidieron, no repitas lo ya dicho.

\`\`\`json
{
  "mensaje1": "Idea principal, muy corta",
  "mensaje2": "",
  "mensaje3": "",
  "mensaje4": "",
  "fotos_mensaje1": [],
...
Reglas de mensajes (BURBUJAS de WhatsApp — como una persona real):
- Cada campo mensaje1/mensaje2/mensaje3/mensaje4 es UNA burbuja: 1 sola idea, corta y directa (1–2 líneas).
- VOS decidís dónde cortar, como cortaría un humano por idea (NO por punto ni coma a la fuerza).
- mensaje1: SIEMPRE va. mensaje2/3/4: opcionales, solo si aportan (pregunta o dato clave). Si no, "".
- ANDÁ AL GRANO: no des contexto de más, no expliques de más, no repitas lo ya dicho.
- En la mayoría de turnos alcanza con mensaje1.
  ...
  const combinedText = recentMessages
    .map(m => {
      if (m.role === 'assistant') {
        try {
          const parsed = JSON.parse(m.content) as Record<string, unknown>
          return [parsed.mensaje1, parsed.mensaje2, parsed.mensaje3, parsed.mensaje4].filter(Boolean).join(' ')
...
export class BotEngine {
  static async handleWebhook(botId: string, payload: Record<string, unknown>): Promise<void> {
    ...
    // 15c. Regla: MÁXIMO 1 imagen y 1 video por mensaje. Se cargan varias solo
    // para tener variedad y rotar (el filtro de arriba ya evita repetir las ya
    // enviadas). Nunca mandar todas juntas.
    response.fotos_mensaje1 = (Array.isArray(response.fotos_mensaje1) ? response.fotos_mensaje1 : []).slice(0, 1)
    response.videos_mensaje1 = (Array.isArray(response.videos_mensaje1) ? response.videos_mensaje1 : []).slice(0, 1)
    ...
    if (!voiceSent && response.mensaje3) {
      ...
      if (response.mensaje4) await sleep(Math.floor(Math.random() * 1000) + 1000) // Retardo humano 1-2s
    }

    if (!voiceSent && response.mensaje4) {
      await sendText(from, toPhone, response.mensaje4, apiKey).catch(e =>
        console.error('[BOT] sendText m4 ERROR:', e.message),
      )
    }
