...
export async function GET(
  ...
  try {
    ...
    const { data: store, error } = await service
      .from('stores')
      .select('id, name, slug, store_type, whatsapp_number, payment_qr_url, visibility, font_family, font_weight, font_spacing, font_style, background_type, background_value, font_config, bg_config, cover_image_url, cover_images, favicon_url')
      .eq('slug', slug)
