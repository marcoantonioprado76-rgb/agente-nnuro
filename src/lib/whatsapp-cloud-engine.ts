...
export class WhatsAppCloudEngine {
  static async handleMessage(
    ...
    await WhatsAppCloudEngine.processBuffered(botId, conversationId)
  }
  ...
  static async processBuffered(botId: string, conversationId: string): Promise<void> {
    if (!acquireBufferLock(conversationId)) {
      console.log(`[WA_CLOUD] processBuffered: conv ${conversationId} ya en proceso, omito`)
      return
    }
    try {
      ...
      if (!bot || bot.status !== 'ACTIVE' || !bot.bot_secrets) return
      const secret = bot.bot_secrets
      if (!secret.meta_page_token_enc || !secret.meta_phone_number_id) return
