      ...
      {!loading && enrollments.length === 0 && (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <Link href="/dashboard/courses"
            ...
            style={{ color: '#147e95', textDecoration: 'none', border: '1px solid #E4E9F0', borderRadius: 8, padding: '7px 16px', display: 'inline-block' }}>
      ...
      {!loading && enrollments.length > 0 && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          {enrollments.map(e => {
            ...
            return (
              <div key={e.id}
                style={{ background: 'linear-gradient(180deg, #212e38 0%, #273842 60%, #1a262f 100%)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 14, overflow: 'hidden' }}>
                ...
                <div style={{ borderTop: '1px solid rgba(255,255,255,0.1)', padding: '10px 16px', display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center' }}>
                  {e.status === 'APPROVED' && (
                    <Link href={`/dashboard/courses/${course.id}`}
                      style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 12, fontWeight: 700, color: '#fff', textDecoration: 'none', background: 'linear-gradient(90deg, #1fb8bb 0%, #147e95 50%, #233B8F 100%)', borderRadius: 8, padding: '7px 14px', whiteSpace: 'nowrap' }}>
                      ▶ Ver videos
                    </Link>
                  )}
                  {e.status === 'REJECTED' && (
