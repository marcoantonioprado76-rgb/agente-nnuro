...
export default function RegisterPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(form.email)) { toast.error('Correo electrónico inválido'); return }
    ...
    if (!form.phoneNumber.trim()) { toast.error('El número de celular es requerido'); return }
        ...
        >
          ...
          <form onSubmit={handleRegister} className="space-y-4">
            ...
            <div className="space-y-2">
              <Label className="text-[11px] font-semibold text-[#94A3B8] uppercase tracking-[0.15em]">Nombre completo</Label>
            ...
            {/* Email */}
            <div className="space-y-2">
              <Label className="text-[11px] font-semibold text-[#94A3B8] uppercase tracking-[0.15em]">Correo electrónico</Label>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-2">
                <Label className="text-[11px] font-semibold text-[#94A3B8] uppercase tracking-[0.15em]">Contraseña</Label>
                <div className="relative">
                  <Input type={showPassword ? 'text' : 'password'} placeholder="Min. 6 chars"
                    ...
                    className="h-12 rounded-xl auth-input pr-10 text-white placeholder:text-[#64748B]/45 text-[14px]" />
                  <button type="button" onClick={() => setShowPassword(!showPassword)}
                    className="absolute right-3 top-1/2 -translate-y-1/2 text-[#64748B]/60 hover:text-white transition-colors">
                    {showPassword ? <EyeOff className="h-3.5 w-3.5" /> : <Eye className="h-3.5 w-3.5" />}
                  </button>
                </div>
              </div>
              <div className="space-y-2">
                <Label className="text-[11px] font-semibold text-[#94A3B8] uppercase tracking-[0.15em]">Confirmar</Label>
                <Input type={showPassword ? 'text' : 'password'} placeholder="Repetir"
            ...
            {/* Country */}
            <div className="space-y-2">
              <Label className="text-[11px] font-semibold text-[#94A3B8] uppercase tracking-[0.15em]">País</Label>
              <Select value={form.country} onValueChange={(v) => {
                if (!v) return
                updateField('country', v)
                const m = countryCodes.find(c => c.country === v)
                if (m) updateField('countryCode', m.code)
              }}>
            ...
            {/* Phone */}
            <div className="space-y-2">
              <Label className="text-[11px] font-semibold text-[#94A3B8] uppercase tracking-[0.15em]">Número de celular</Label>
              <div className="flex gap-2">
                <Select value={form.countryCode} onValueChange={(v) => { if (v) updateField('countryCode', v) }}>
                  <SelectTrigger className="w-[130px] h-12 rounded-xl auth-input text-white">
                    <SelectValue>
                      {selectedCountry ? `${selectedCountry.flag} ${selectedCountry.code}` : form.countryCode}
                    </SelectValue>
                  </SelectTrigger>
                  <SelectContent>
                    {countryCodes.map(c => (
                ...
                <Input type="tel" placeholder="67534487" value={form.phoneNumber}
                  ...
                  className="flex-1 h-12 rounded-xl auth-input text-white placeholder:text-[#64748B]/45 text-[14px]" />
              </div>
            </div>
